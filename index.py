...
def update_shards(desiredCapacity, resourceName):
    ...
    try:
        ...
        print("Update shards to: "+str(desiredCapacity))
...
def update_alarm_out(shards, stream):
    ...
    new_records_threshold = (1000 * int(shards) * 60)*80/100
    # assuming alarm will fire at 80% of incoming records
    new_bytes_threshold = (1000000 * int(shards) * 60)*80/100
    try:
        ...
        AUTOSCALINGPOLICYOUT_ARN
            ...
            Metrics=[{
                    'Id': 'e1',
                    'Label': 'Should-Scale-Out',
                    'ReturnData': True,
                    'Expression': 'IF(m1>'+str(new_bytes_threshold)+' OR m2>'+str(new_records_threshold)+' OR m3>0, 1, 0)',
    }, {
        'Id': 'm1',
        'ReturnData': False,
        'MetricStat': {
...
def update_alarm_in(shards, stream):
    # assuming alarm will fire at 80% of incoming records
    new_records_threshold = (1000 * (int(shards)-1) * 60)*80/100
    # assuming alarm will fire at 80% of incoming records
    new_bytes_threshold = (1000000 * (int(shards)-1) * 60)*80/100
    try:
        set_alarm = client_cloudwatch.put_metric_alarm(
        ...
        'Label': 'Can Scale-In',
        'ReturnData': True,
        'Expression': 'IF(m1<'+str(new_bytes_threshold)+' AND m2<'+str(new_records_threshold)+', 1, 0)',
    }, {
        'Id': 'm1',
        'ReturnData': False,
        'MetricStat': {
