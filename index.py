...
def update_alarm_out(shards, stream):
	new_threshold = (1000 * int(shards) * 60)*80/100 #assuming alarm will fire at 80% of incoming records
	try: 
...
def update_alarm_in(shards, stream):
	new_threshold = (1000 * (int(shards)-1) * 60)*80/100 #assuming alarm will fire at 80% of incoming records
	try:
		set_alarm = client_cloudwatch.put_metric_alarm(
